...
@section('content')
    <div class="container-xl">
        <div class="row">
            <div class="col-12">
                <x-card.card title="Edit Kursus">
                    <form action="{{ route('admin.series.update', $series->slug) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <x-form.input type="text" title="Nama Kursus" name="name" value="{{ $series->name }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-6">
                                <x-form.input type="number" title="Harga Kursus" name="price" value="{{ $series->price }}"
                                    placeholder="Masukkan Harga Kursus" />
                            </div>
                        </div>
                        <img src="{{ $series->cover }}" alt="{{ $series->name }}" class="img-fluid mb-3 " width="20%" />
                        <x-form.input type="file" title="Gambar Kursus" name="cover" value="{{ $series->cover }}"
                            placeholder="" />
                        <x-form.textarea title="Deskripsi Kursus" name="description" placeholder="">
                            {{ $series->description }}
                        </x-form.textarea>
                        <div class="row">
                            <div class="col-6">
