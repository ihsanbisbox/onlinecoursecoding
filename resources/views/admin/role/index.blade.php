    ...
    <div class="container-xl">
        <div class="row">
                                    ...
                                    <td>
                                        ...
                                        <x-modal.modal title="Edit Role" id="{{ $role->id }}">
                                            <form action="{{ route('admin.roles.update', $role->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
                                                <x-form.input type="text" title="Nama Role" name="name"
                                                    placeholder="Edit Nama Role" value="{{ $role->name }}" />
            ...
            <div class="col-12 col-lg-4">
                <x-card.card title="Tambahkan Role Baru">
                    <form action="{{ route('admin.roles.store') }}" method="POST">
                        @csrf
                        <x-form.input type="text" title="Nama Role" name="name" placeholder="Masukkan Nama Role" value="" />
                        <x-form.select-group title="Permissions">
                            @foreach ($permissions as $permission)
                                <label class="form-selectgroup-item">
