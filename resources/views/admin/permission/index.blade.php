    ...
    <div class="container-xl">
        <div class="row">
                                    ...
                                    <td>
                                        ...
                                        <x-modal.modal title="Edit Permission" id="{{ $permission->id }}">
                                            <form action="{{ route('admin.permissions.update', $permission->id) }}"
                                                ...
                                                @csrf
                                                @method('PUT')
                                                <x-form.input type="text" title="Nama Permission" name="name"
                                                    placeholder="Edit Nama Permission" value="{{ $permission->name }}" />
            ...
            <div class="col-12 col-lg-4">
                <x-card.card title="Tambahkan Permission Baru">
                    <form action="{{ route('admin.permissions.store') }}" method="POST">
                        @csrf
                        <x-form.input type="text" title="Nama Permission" name="name" placeholder="Masukkan Nama Permission"
                            value="" />
                        <x-button.button-save icon="save" title="Simpan" class="btn btn-primary" />
                    </form>
                </x-card.card>
            </div>
        </div>
